...
class TcpClient
{
    ...
    {
        if (in_array(ucfirst($this->module), ['Search', 'Recommend'])) {
            $config = $this->config['ServiceSearchRecommend'];
        } else {
            $config = $this->config[ucfirst($this->module)];
        }

        if (empty($config) || !isset($config['host']) || !isset($config['port']) || !isset($config['secretKey']) || !isset($config['account'])) {
